<script lang="ts">
    ...
    async function loadProducts() {
        ...
        loadError = null;
        try {
            const res = await fetch('http://16.171.151.62:8080/products');
    ...
    async function handleRegister() {
        ...
        try {
            const res = await fetch('http://16.171.151.62:8080/register', {
                method: 'POST',
    ...
    async function handleConfirm() {
        try {
            const res = await fetch('http://16.171.151.62:8080/confirm', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    try {
        ...
        const res = await fetch('http://16.171.151.62:8080/change-password', {
            method: 'POST',
            headers: { 
    ...
    async function handleResend() {
        try {
            const res = await fetch('http://16.171.151.62:8080/resend-confirm', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function handleLogin() {
        ...
        try {
            const res = await fetch('http://16.171.151.62:8080/login', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function handleFavourites() {
        try {
            ...
            const res = await fetch("http://16.171.151.62:8080/favourites", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async function addFavourite(deal) {
        try {
            ...
            const res = await fetch("http://16.171.151.62:8080/addfavourite", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
    ...
    async function removeFavourite(deal) {
        try {
            ...
            const res = await fetch("http://16.171.151.62:8080/removefavourite", {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    userId: String(userId),
...
.modal-basic-info {
  ...
  @media (max-width: 768px) {
    ...
    /* suggestions dropdown positioned below the header search input */
    .header-suggestions {
      position: absolute;
      top: calc(100% + 8px); /* directly under the input */
      left: 0;
      width: 100%;
      max-width: 640px; /* match input width */
      background: white;
    ...
    @media (max-width: 800px) {
        .header-search {
            display: none;             /* hide search bar on tiny screens */
        }
    }
    .sub-header {
        height: 32px;
...
<div class="min-h-screen w-full" style="background: linear-gradient(90deg, #e0e7ff 0%, #f0fdfa 100%);">
    ...
    <header class="fixed top-0 left-0 z-50 px-6 py-4 bg-white shadow w-full flex items-center">
        ...
        <div class="header-search">
            ...
            />
            {#if Headersearch}
                <button class="clear-btn-header" on:click={() => search = Headersearch = ''}>×</button>
            {/if}

            {#if Headersuggestions.length > 0}
                <div class="header-suggestions">
                    {#each Headersuggestions as s}
                        <div
                            class="item"
                            on:click={() => {
                                search = s.title;
                ...
                <div class="search-box relative">
                    ...
                    <div class="mb-2 mt-8">
                        ...
                        <div>
                            ...
                            />
                            {#if search}
                                <button class="clear-btn" on:click={() => search = Headersearch= ''}>×</button>
                            {/if}
                        </div>
                    </div>
